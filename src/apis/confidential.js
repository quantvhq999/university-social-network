...
export const postConfidentialComment = async (data) => {
    let enpoint = API_URL + 'confidential/comment/' + data.id
    let contents = {
        content: data.content 
    }
    return await axiosServices.post(enpoint, contents).then(res => {
        return res.data
    })
}